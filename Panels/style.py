...
class ICECUBERIG_PT_StyleTaper(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = col.row(align=True)
        row.prop(obj,"upper_body_taper",text="Upper Body Taper",slider=True)
...
def toggleButton(obj, pos, propName, text):
    ...
    is_toggled = getattr(obj, propName, True)

    icon = "CHECKBOX_HLT" if is_toggled else "CHECKBOX_DEHLT"

    context_show_hide = "Hide" if is_toggled else "Show"
    display_text = text.replace("s/h", context_show_hide.lower())
    display_text = display_text.replace("S/H", context_show_hide.upper())

    pos.prop(obj, propName, text=display_text, icon=icon, toggle=True)
...
class ICECUBERIG_PT_StyleEmotions(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if toggleButton(obj, box, "Wrinkles", "Wrinkles"):
            c1 = box.column(align=True)
            g1 = c1.grid_flow(columns=1, align=True)
            
            if 'Wrinkles' in pose_bones:
                g1.prop(pose_bones['Wrinkles'], '["Frown"]', text='Frown', toggle=True)
                g1.prop(pose_bones['Wrinkles'], '["Eyebrow Frown"]', text='Eyebrow Frown', toggle=True)
                g1.prop(pose_bones['Wrinkles'], '["Shocked"]', text='Shocked', toggle=True)
                g1.prop(pose_bones['Wrinkles'], '["Eyelid Side T"]', text='Eyelid Side T', toggle=True)
                g1.prop(pose_bones['Wrinkles'], '["Eyelid Middle T"]', text='Eyelid Middle T', toggle=True)
                g1.prop(pose_bones['Wrinkles'], '["Eyelid Side B"]', text='Eyelid Side B', toggle=True)
                g1.prop(pose_bones['Wrinkles'], '["Eyelid Middle B"]', text='Eyelid Middle B', toggle=True)
                g1.prop(pose_bones['Wrinkles'], '["Eyes Bag"]', text='Eyes Bag', toggle=True)
                g1.prop(pose_bones['Wrinkles'], '["Tear Trough"]', text='Tear Trough', toggle=True)
...
class ICECUBERIG_PT_StyleMaterial(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if eye_node.inputs[5].default_value:
            ...
            eyes_row = eyes_box.row(align=True)
            eyes_row.prop(eye_node.inputs[44],'default_value',text="Sparkle Emission",toggle=True)
            eyes_row = eyes_box.row(align=True)
        ...
        if eye_node.inputs[42].default_value or eye_node.inputs[43].default_value :
            eyes_row.prop(eye_node.inputs[45],'default_value',text="Real Light (Cycles Only)")
            eyes_row.prop(eye_node.inputs[46],'default_value',text="Visual Strength")
            eyes_row = eyes_box.row(align=True)
